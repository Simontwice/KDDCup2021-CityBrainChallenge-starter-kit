...
class TestAgent():
    ...
    def inverse_traffic_signal_dataset(self):
        for key, val in self.road_map.items():
            exiting_roads = val['exit_order']
            if exiting_roads == []:
                continue
            for i, road in enumerate(exiting_roads):
                if road != -1:
                    intersection = self.roads[road]['end_inter']
                    enter_road = self.road_map[intersection]['exit_order']
                    i = [2, 3, 0, 1][i]

                    if enter_road == []:
                        enter_road = set(self.road_map[intersection].keys())
                        enter_road.remove('approach_order')
                        enter_road.remove('exit_order')
                        enter_road.remove(road)
                        enter_road = list(enter_road)
                        print(enter_road)
                        # enter_road = enter_road[0]
                    else:
                        enter_road = enter_road[i]
                else:
                    enter_road = -1
                self.road_map[key]['approach_order'].append(enter_road)
    ...
    def vehicle_info_map(self, info):
        ...
        for _, val in info.items():
            ...
            if lane_dir not in vehicle_info[intersection_id][road_id].keys():
                vehicle_info[intersection_id][road_id][lane_dir] = {'waiting': 0, 'approaching': 0}

            # calculating waiting cars by speed <= 0
            if val['speed'][0] <= 0:
                vehicle_info[intersection_id][road_id][lane_dir]['waiting'] += 1
            else:
                vehicle_info[intersection_id][road_id][lane_dir]['approaching'] += 1
        return vehicle_info
            ...
            for road_key in self.road_map[intersection_key].keys():
                ...
                for lane_key in range(self.road_map[intersection_key][road_key]['num_lanes']):
                    ...
                    try:
                        ...
                        new_approaching_car = vehicle_info[intersection_key][road_key][lane_key]['approaching']
                    except KeyError:
                        new_waiting_car = 0
                        new_approaching_car = 0

                    old_waiting_car = self.road_map[intersection_key][road_key][lane_key]['waiting'][-1]
                    old_approaching_car = self.road_map[intersection_key][road_key][lane_key]['approaching'][-1]
                    ...
                    self.road_map[intersection_key][road_key][lane_key]['approaching'].append(new_approaching_car)
    ...
    def vector_of_approaching(self, agent):
        vector_of_congestion = []
        road_order = self.road_map[agent]['approach_order']
        for road in road_order:
            if road == -1:
                ...
                for lane in range(3):
                    if lane not in self.road_map[agent][road].keys():
                        ...
                        cars = self.road_map[agent][road][lane]['approaching'][-1]
                    vector_of_congestion.append(cars)
        return vector_of_congestion
            ...
            if road == -1:
                ...
                for lane in range(3):
                    if lane not in self.road_map[agent][road].keys():
                        ...
                        cars = self.road_map[agent][road][lane]['waiting'][-1] + \
                               self.road_map[agent][road][lane]['approaching'][-1]
                    vector_of_congestion.append(cars)
        return vector_of_congestion
    ...
    def act(self, obs):
        ...
        for agent in self.agent_list:
            ...
            assert (observations_for_agent[agent]["lane_vehicle_num"][1:13] == self.vector_of_congestion(agent))
