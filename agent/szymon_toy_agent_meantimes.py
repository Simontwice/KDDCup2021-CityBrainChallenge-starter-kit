...
with open(path + "/gym_cfg.py", "r") as f:
    pass
#HYPERPARAMETERS: BASE_CYCLE_DURATION, decay_multiplier, decay_interval
def weight_function(x):
    if x>=0:
        return (12.65 - (8*x) ** 0.5)*(20 - x) * 0.05
    else:
        return 0
...
class TestAgent():
    def __init__(self):
        self.now_phase = {}
        self.base_cycle_duration = 10*12
    ...
    def default_cycle_time_decay(self, current_time):  # experimental: alter the def cycle duration as time progresses
        ...
        if current_time > 1200:
            ...
            if current_time > 2400:
                time += 10
        self.default_cycle_duration = time
    def act(self, obs):
        """ !!! MUST BE OVERRIDED !!!
        """
        ...
        for agent in self.agent_list:
            ...
            if agent not in self.agent_plan_priorities:
                self.agent_plan_priorities[agent] = -1
            ...
            if now_step >= self.next_cycle_plan[agent] or np.max(self.agent_phase_plan[agent]) <= 0:  # if it's time to get a new plan or all the values in the plan are negative
                ...
                vector_of_speeds = observations_for_agent[agent][
                    "lane_speed"]
                phases = []
                priorities = []
                for i in range(1, self.max_phase + 1):
                    relevants = [vector_of_congestion[j] for j in self.lanes_by_phase[i]]
                    speeds_temp = [weight_function(vector_of_speeds[j]) for j in self.lanes_by_phase[i]]
                    speeds_temp = np.asarray(speeds_temp)
                    relevants = np.asarray(relevants)
                    relevants[relevants <= 0] = 0  # some values will be negative since there are three-legged interestions, and they put negative values in those places
                    suma = np.sum(relevants)
                    phases.append(suma)
                    priorities_temp = np.sum(speeds_temp*relevants/suma)
                    priorities.append(priorities_temp)
                ...
                if not no_cars:
                    ...
                    np_phases *= self.default_cycle_duration * (
                                np.count_nonzero(np_phases) / self.max_phase)  # stretch over def_cycle_duration
                    np_phases = smooth_phase_times(np_phases)
                    this_cycle_duration = np.sum(np_phases) + np.count_nonzero(
                        np_phases) * 5  # mind the 5 sec all red intermissions!
                    self.agent_phase_plan[agent] = np_phases
                    self.next_change_step[
                        agent] = now_step - 1  # its a new plan, time to use it immediately (described below)
                    self.next_cycle_plan[agent] = now_step + this_cycle_duration

            if no_cars:
                self.next_cycle_plan[agent] = now_step # change plan v quickly, cars will probably appear shortly
            # EXECUTE PLAN
            if now_step >= self.next_change_step[agent]:  # if it's time to change phases according to the current plan
                if np.max(self.agent_phase_plan[agent]) > 0:
                    argmax_of_list = (self.agent_plan_priorities[agent]).argmax()
                    assert(self.agent_phase_plan[agent][argmax_of_list]>0)
                    self.now_phase[agent] = argmax_of_list + 1  # since phase 1 is on position 0, phase 2 on pos 1 etc.
                    self.next_change_step[agent] = now_step + self.agent_phase_plan[agent][argmax_of_list] # if it somehow happened that all values in the plan are negative, get 0. Else get the biggest one, then set it to -10
                    self.agent_phase_plan[agent][argmax_of_list] = -10
                    self.agent_plan_priorities[agent][argmax_of_list] = -10
                    actions[agent] = self.now_phase[agent]
                else:
                    #print("no plan, yet tried to execute")
                    self.next_cycle_plan[agent] = now_step #if all the phases had neg values, time to get a new plan

        print("cycle_duration: ",self.default_cycle_duration)
        return actions
