...
class TestAgent():
    ...
    def load_roadnet(self, intersections, roads, agents):
        ...
        for agent in self.agent_list:
            ...
            self.intersection_traffic[agent]=np.zeros((12,1))
    ...
    def aggregate_traffic_data(self,rewards):
        for id,val in rewards.items():
            self.intersection_traffic[id] = np.concatenate((self.intersection_traffic[id],np.asarray([i if isinstance(i, int) else i[0] for i in val[0:12]]).reshape(12,-1)),axis=-1)
    ...
    def act(self, obs):
        ...
        for agent in self.agent_list:
            ...
            if self.now_step >= self.next_cycle_plan[
                agent]:  # if it's time to get a new plan
