...
class TestAgent():
    def __init__(self):
        ...
        self.default_cycle_duration = self.base_car_time # duration of 1 CYCLE (also called 1 plan) that will be partitioned according to needs on the intersect
        ...
        for agent in self.agent_list:
            ...
            if now_step >= self.next_cycle_plan[agent]:  # if it's time to get a new plan or all the values in the plan are negative
                ...
                if not no_cars:
                    ...
                    np_phases*=self.base_car_time
                    np_phases = smooth_phase_times(np_phases)
                    this_cycle_duration = np.sum(np_phases)+(np.count_nonzero(np_phases) - 1)*5 #mind the 5 sec all red intermissions!
