...
class TestAgent():
    ...
    def default_car_time_decay(self, current_time):  # experimental: alter the def cycle duration as time progresses
        self.base_car_time = self.base_car_time + self.delay[int(current_time / 300)]
    ...
    def get_congestion(self,agent):
        time = self.now_step
        if time < 800:
            return self.vector_of_congestion(agent)
        else:
            if time < 1400:
                return self.vector_of_waiting(agent) + self.vector_of_approaching_close(agent)
            else:
                return self.vector_of_waiting(agent)


    def get_priorities(self,agent):
        time = self.now_step
        if time < 800:
            return self.inv_vector_of_congestion(agent)
        else:
            if time < 1400:
                return self.inv_vector_of_waiting(agent) + self.inv_vector_of_approaching_close(agent)
            else:
                return self.inv_vector_of_waiting(agent)


    def create_new_plan(self, agent):
        vector_of_congestion = self.get_congestion(agent)
        vector_of_priorities = self.get_priorities(agent)
        phases = []
        priorities = []
        no_cars = False
        for i in self.suitable_phases[self.agent_topology[agent]]:
            relevants = [vector_of_congestion[j] for j in self.lanes_by_phase[i]]
            priorities_temp = [vector_of_priorities[j] for j in self.lanes_by_phase[i]]
            ...
            np_pr = np.asarray(priorities_temp)
            np_pr[
                np_pr <= 0] = 0
            suma = np.sum(np_rel)
            suma_pr = np.sum(np_pr)
            phases.append(suma)
            priorities.append(suma_pr)
        if max(phases) <= 0:
            no_cars = True
        if not no_cars:
            ...
            this_cycle_duration -= 5 * int(self.current_phase[agent] == self.suitable_phases[self.agent_topology[agent]][np.argmax(np_phases)])
            self.agent_phase_plan[agent] = np_phases
            self.agent_priorities[agent] = np_priorities
    ...
    def change_phase(self, agent):
        if self.now_step < 800:
            index = np.argmax(self.agent_phase_plan[agent])
        else:
            index = np.argmax(self.agent_priorities[agent])
        agent_topology = self.agent_topology[agent]
        phase_list = self.suitable_phases[agent_topology]
        ...
        self.agent_priorities[agent][index] = -10
    ...
    def act(self, obs):
        ...
        for agent in self.agent_list:
            ...
            if agent not in self.agent_phase_plan:  # if we don't have plans, also only at the beginning, put a dummy plan that will immediately be changed
                ...
                self.agent_priorities[agent] = np.asarray([-100])
            ...
            if self.now_step >= self.next_cycle_plan[
                ...
                no_cars = self.create_new_plan(agent)
                if no_cars:
